...
def train_testID_returnSampling(inputID_file, tfX, ratioT = 0.15):
    ...
    print(len(idTest), len(idValid), len(idTrain))
...
def make_model( hp ):
    ...
    model = keras.models.Sequential([
        ...
        keras.layers.Conv1D(
            960,
            hp.Int('kmer' + str(i + 2), 6,10,1), 
            activation = hp.Choice('ConvAct_' + str(i + 2), ['relu', 'tanh']),
            padding = hp.Choice('ConvPadding' + str(i + 2), ['valid','same']),
            kernel_regularizer=regularizers.l2(hp.Float('convL2' + str(i + 2), 1e-10, 1e-4,sampling='log')),
            kernel_constraint=max_norm(hp.Float('max_norm_1stConv' + str(i + 2), 0.1,3.1,0.5))
        ),
        keras.layers.MaxPooling1D(
            hp.Int('maxPool' + str(i + 2), 2,6,1), 
            strides = hp.Int('maxPool_strid' + str(i + 2), 2,6,1)
        ),
        keras.layers.Dropout(hp.Float('drop_' + str(i + 2), 0.1, 0.9, 0.1)),
    ...
    model.compile(
            optimizer=keras.optimizers.Adam(lr=hp.Choice('ADAM',[1e-3,1e-4,1e-5])),
...
def main():
    ...
    (tfXtrainPos, tfXvalidPos, tfXtestPos) = train_testID_returnSampling('../time_specific_peak_generateTensor/tensorData_h15_20/locName.positive_1k.txt', trainAllpos) #test valid
    ...
    print('------neg------')
    ##sampling for test#############################################
    #idSample = sample(range(tfXtrainPos.shape[0]), 100)
    #tfXtrainPos = tf.gather(tfXtrainPos, indices = idSample)
    ################################################################
    #print(tfXtrainPos.shape)
    #print(trainAllneg.shape)
    ...
    print(testX.shape)
    print(tfXtrainPos.shape)
    print(tfXtrainNegSample.shape)
    print(tfXtestPos.shape)
    print(tfXtestNegSample.shape)
    #print(valiY.shape)
    #print(valiX.shape)
