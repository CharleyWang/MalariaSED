...
#each row is ID in inputID_file, tfX is the tensor with cresponding name in inputID_file
def train_testID_return(inputID_file, tfX, testID = [['chr10:','chr11:'], ['chr12:','chr9']]):
    idTest = []
    idValid= []
    idTrain= []
    ...
    for l in lines:
        t = 0; v = 0;
        if testID[0][0] in l or testID[0][1] in l:
                v = v + 1
        elif testID[1][0] in l or testID[1][1] in l:
                t = t + 1
        if t == 0 and v == 0:
            idTrain.append(idV);
        elif t == 0:
...
def main():
    ...
    (tfXtrainPos, tfXvalidPos, tfXtestPos) = train_testID_return('../time_specific_peak_generateTensor/tensorData_h15_20/locName.positive_1k.txt',
            trainAllpos)
    (tfXtrainNeg, tfXvalidNeg, tfXtestNeg) = train_testID_return('../time_specific_peak_generateTensor/tensorData_h15_20/locName.negative_1k.txt',
            trainAllneg)
    ...
    EPOCHS = 3000
    BATCH_size = 2000
    #model = make_model(np.log(0.0196))
    #print(model.summary())
